...
export var alpha_decay_per_second : float = 0.075

var alpha_decay : float = 0
...
func _physics_process(delta):
    ...
    if is_active():
        ...
        if is_growing and !exploded:
            start_explode()
    elif !$ExplosionSound.playing:
        queue_free()

func update_alpha(delta):
    alpha_decay += alpha_decay_per_second * delta
    material.set_shader_param("alpha_decay", alpha_decay)

func is_active():
    return time_passed < explosion_duration
...
func _on_Area_body_entered(body):
    if body.has_method("add_flying_force") and body != Globals.get_player() and is_growing and is_active():
